...
| 日期 | 版本号 | 类型 | 更新内容 |
| --- | --- | --- | --- |
| 2026-08-31 | v2.4.0 | 功能与架构 | 新增智联招聘渠道（采集+评分+招呼语生成全通，发送暂缓）。智联新版搜索页把完整岗位数据内联在 __INITIAL_STATE__，采集直接直读（真实薪资/全文 JD/公司信息），免点击、免开详情页、未登录可用；薪资解析支持「万/月」与「元/月」口径；渠道支持翻页上限与免详情页开关。 |
