...
| 日期 | 版本号 | 类型 | 更新内容 |
| --- | --- | --- | --- |
| 2026-08-31 | v2.4.0 | 功能与架构 | 新增智联招聘渠道全通（采集+评分+招呼语生成+投递+招呼语发送）。智联新版搜索页把完整岗位数据内联在 __INITIAL_STATE__，采集直接直读（真实薪资/全文 JD/公司信息），免点击、免开详情页；薪资解析支持「万/月」与「元/月」口径；智联发送实现「投递+招呼语」专属链路（选简历投递 → 平台默认招呼语 → 会话内发送自定义招呼语），已实机验证真实投递与会话发送；渠道支持翻页上限与免详情页开关；支持多启用渠道并行采集（channels.active 可配置为列表），岗位在数据库与前端工作台均标注来源渠道（Boss直聘/智联招聘）。 |
