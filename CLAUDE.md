...
当用户要求"启动"、"开始"、"run"、"投递"时，**不要直接执行 `jobwinner run`**，必须先完成以下检查：
...
>
> 请先完成简历上传：
> - 打开配置面板：`jobwinner web` → http://127.0.0.1:8686
...
**只有确认简历是用户本人的真实简历后，才可以执行 `jobwinner run`。**
...
```bash
jobwinner web              # 打开 Web 配置面板
jobwinner connect          # 检测 Chrome CDP 连接
jobwinner ai-status        # 安全检测 AI API 连接（不显示 Key）
jobwinner run              # 一键执行完整流程
jobwinner scrape -k "关键词"  # 采集岗位
jobwinner score            # AI 评分
jobwinner greet            # 生成招呼语
jobwinner confirm          # 人工确认
jobwinner send             # 发送
jobwinner monitor          # 监听 HR 回复
jobwinner status --full    # 查看状态
```
