...
def scrape_jobs(
    ...
    *,
    ...
    # Resolve cities: search.cities > profile.target_cities > ["北京"]（全部启用渠道共用）
    ...
    with Progress(
        ...
        # 每个启用渠道各自：城市码表不同、翻页上限不同、详情页策略不同
        for channel in channels:
            ...
            # Build per-channel search combinations: city × keyword
            search_combos = []
            for city in cities:
                city_code = _resolve_city_code(city, config, channel)
                if not city_code:
                    console.print(f"[yellow]⚠ {channel.label} 未识别的城市: {city}，已跳过[/yellow]")
                    continue
                for keyword in keywords:
                    search_combos.append((city, city_code, keyword))

            if not search_combos:
                console.print(f"[yellow]{channel.label}: 没有有效的搜索组合（检查城市配置），跳过该渠道[/yellow]")
                continue

            console.print(f"[dim]{channel.label}: 搜索组合 {len(search_combos)} 个 ({len(cities)}城市 × {len(keywords)}关键词 × {max_pages}页)[/dim]")
            ...
            for city, city_code, keyword in search_combos:
                ...
                label = f"{channel.label}·{city}/{keyword}" if len(cities) > 1 else f"{channel.label}·{keyword}"
