...
设计约束：
- 需要用户在 JobWinner Chrome 中已登录对应官网校招账号，否则页面跳转登录页，巡检自动跳过。
...
def _safe_close(tab_id: str) -> None:
    try:
        from jobwinner.browser import close_tab
...
def check_single_record_page(
    ...
    *,
    ...
    from jobwinner.browser import evaluate, get_page_info, new_tab
...
def _update_job_stage(
    ...
    from jobwinner.db import get_db, set_job_stage
