...
from jobwinner.ai.credentials import call_anthropic_text
from jobwinner.browser import close_tab, new_tab, print_pdf
from jobwinner.cancellation import OperationCancelled, run_cancellable, stop_requested
from jobwinner.db import get_db

console = Console()

RESUME_COMPLETION_MARKER = "<!-- JOBWINNER_RESUME_DONE -->"
...
def _render_pdf_via_cdp(html_content: str, output_path: Path) -> bool:
    ...
    temp_html = Path(tempfile.gettempdir()) / "jobwinner_resume.html"
...
def generate_tailored_resume(job_id: str, config: dict) -> Path | None:
    ...
    from jobwinner.ai.resume_picker import select_resume_path_for_job
