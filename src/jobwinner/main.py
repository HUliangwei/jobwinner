...
console = Console()
GITHUB_URL = "https://github.com/HUliangwei/jobwinner"


def _hint_web():
    """Print a one-line hint about the web dashboard."""
    console.print("[dim]💡 运行 jobwinner web 可打开可视化看板[/dim]")
...
def _hint_star_support(base_dir: Path | None = None):
    ...
    if marker.exists():
        return
    console.print(f"[dim]⭐ 如果 JobWinner 帮你节省了求职时间，欢迎 Star 支持继续维护：{GITHUB_URL}[/dim]")
    try:
...
def _prompt_setup(port: int = 8686) -> None:
    ...
    console.print()
    console.print("[bold cyan]═══ 欢迎使用 JobWinner ═══[/bold cyan]")
    console.print()
    console.print("[yellow]检测到尚未配置，建议先进入 Web 端完成初始设置：[/yellow]")
    console.print()
    console.print(f"  [bold]jobwinner web[/bold]  →  打开配置面板 (http://127.0.0.1:{port})")
    console.print()
...
@click.group(name="jobwinner", invoke_without_command=True)
@click.version_option(version=__version__, prog_name="jobwinner")
@click.option("--config", "config_path", default=None, type=click.Path(exists=False), help="配置文件路径（默认 config.yaml）")
@click.pass_context
def cli(ctx: click.Context, config_path: str | None) -> None:
    """JobWinner - 某直聘智能求职Agent"""
    ...
    from jobwinner.browser import configure
    ...
    if ctx.invoked_subcommand is None:
        if _is_first_run(path):
            ...
            console.print("[bold cyan]JobWinner[/bold cyan] 已就绪")
            console.print("[dim]运行 jobwinner --help 查看可用命令[/dim]")
            _hint_web()
...
def connect(ctx: click.Context) -> None:
    """检测 JobWinner Browser Runtime 与 Chrome 连接"""
    from jobwinner.browser import configure
...
def ai_status(ctx: click.Context) -> None:
    ...
    from jobwinner.web.preflight import check_ai_connection
...
def status(ctx: click.Context, full: bool) -> None:
    """查看投递状态统计"""
    if full:
        from jobwinner.tracker.status import show_dashboard
        show_dashboard()
    else:
        from jobwinner.tracker.status import show_status
        show_status()
...
def run(ctx: click.Context) -> None:
    ...
    from jobwinner.pipeline import run_pipeline

    config = ctx.obj["config"]
    console.print("[bold cyan]═══ JobWinner 启动 ═══[/bold cyan]\n")
    run_pipeline(config)
    _hint_web()
    _hint_star_support()
...
def check_portal(ctx: click.Context, wait: int) -> None:
    ...
    需要 Chrome 已通过 9222 端口连接（jobwinner connect 可检测）。
    """
...
def web(ctx: click.Context, port: int, no_open: bool) -> None:
    ...
    from jobwinner.web.server import run_server, set_base_dir

    set_base_dir(ctx.obj["base_dir"])
    console.print("[bold cyan]═══ JobWinner Web Dashboard ═══[/bold cyan]")
